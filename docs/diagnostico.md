...
4.1. El valor de la duración cambió de 1m aproximadamente a 1m 28 s. Puede deberse a los pasos adicionales que se han agregado.  

4.2.Desde el tag v1.2.0 se identifican tres tipos de cambio: dos feat sobre el código de negocio (0b981ca, b7e44ce) y cambios de tipo ci/chore sobre .github/workflows/pipeline.yml. 
...
Otra limitación real es que el pipeline fija las dependencias en requirements.lock (con versiones exactas), pero eso solo garantiza reproducibilidad, no seguridad: nada en el pipeline verifica si alguna de esas versiones pineadas tiene una vulnerabilidad conocida (CVE) publicada después de haberlas fijado. Podrías estar "congelando" una versión insegura de requests o urllib3 indefinidamente sin que el pipeline lo detecte, porque SonarCloud analiza código propio, no hace análisis de composición de software (SCA) sobre las dependencias de terceros. Para resolverlo haría falta agregar un paso de escaneo de dependencias que falle el job validar si detecta una vulnerabilidad de severidad alta en algún paquete pineado, y que además abra alertas automáticas cuando aparezcan nuevas vulnerabilidades para versiones ya fijadas en el lock file.
...
Respecto a la intervención (el SUCCESS), el pipeline sigue teniendo limitaciones. Nombren una y expliquen qué haría falta para resolverla. Respuesta breve y en párrafo.
